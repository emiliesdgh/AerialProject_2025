...
def get_command(sensor_data, camera_data, dt):
    ...
    if sensor_data['x_global'] < 2.5 :
        feed, pink_center = detect_pink(camera)
        if pink_center is not None:
            print("y from center: ", pink_center[0] - camera_data.shape[1] // 2)
            print("z from center: ", pink_center[1] - camera_data.shape[0] // 2)
...
def detect_pink(camera_data):
    ...
    HSV_img = cv2.cvtColor(camera_data, cv2.COLOR_BGR2HSV)
    ...
    if max_contour is not None :
        cv2.drawContours(HSV_img, [max_contour], -1, (0, 255, 0), 2)

        moment = cv2.moments(max_contour)
        cx = int(moment["m10"] / moment["m00"])
        cy = int(moment["m01"] / moment["m00"])
        center_point = (cx, cy)
        cv2.circle(HSV_img, (cx, cy), 2, (0, 0, 255), -1)

        # Draw middle lines of the img 
        cv2.line(HSV_img, (HSV_img.shape[1]//2, 0), (HSV_img.shape[1]//2, HSV_img.shape[0]), (0, 255, 0), 1)
        cv2.line(HSV_img, (0, HSV_img.shape[0]//2), (HSV_img.shape[1], HSV_img.shape[0]//2), (0, 255, 0), 1)

    return HSV_img, center_point
