...
def get_command(sensor_data, camera_data, dt):
    global start_time, F_PIXEL, position, WayPoints
    global deltaT, control_command, firstWaypoint, secondWaypoint
    ...
    if sensor_data['z_global'] > 0.9 :#and start_position == True:
    ...
    # # if sensor_data
    # while True and not start_position:
    #     print("in while")
    #     # control_command = [0.75, 3.5, 1.0, sensor_data['yaw']+0.1]
    #     time.sleep(2)
    #     position = 'SECOND'
    ...
    if sensor_data['x_global'] < 0.8 and position == 'SECOND':
        second_position = True
    

    if position == 'SECOND' and second_position == True:
    ...
    if position == 'THIRD' and WayPoints == 'TWO':
...
def get_waypoint(sensor_data, camera, deltaTime):
    ...
    ## once drone stable enough, get starting position information ##
        ...
        # elif secondWaypoint:
        #     next_pos = np.array([initial_pos[0]+0.5, initial_pos[1]-0.5, 1.0, initial_pos[3]+0.1])
    ...
    if deltaTime > deltaT+6.5 and deltaTime < deltaT+8:
        initial_pos = np.array([sensor_data['x_global'], sensor_data['y_global'], sensor_data['z_global'], sensor_data['yaw']])
        control_command = [waypoint[0], waypoint[1], waypoint[2], sensor_data['yaw']]
        # control_command = [waypoint[0], waypoint[1], waypoint[2], alpha]
